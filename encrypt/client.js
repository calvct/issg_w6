...
socket.on("message", (data) => {
  ...
  if (senderUsername !== username) {
    if (targetUsername === username) {
      const plaintext = crypto.privateDecrypt(senderPrivateKey, senderMessage);
      console.log(`${senderUsername}: ${plaintext.toString()}`);
    } else {
      console.log(`${senderUsername}: ${senderMessage.toString()}`);
    }
    rl.prompt();
  }
});
